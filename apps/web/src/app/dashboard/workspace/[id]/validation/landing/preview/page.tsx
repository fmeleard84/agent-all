...
import {
  ...
  Loader2, Quote, CheckCircle, Mail, Phone, MapPin, ArrowLeft,
} from 'lucide-react'
...
export default function LandingPreview() {
  ...
  return (
    <div style={{ fontFamily: `${b.bodyFont || 'Inter'}, system-ui, sans-serif` }} className="bg-white text-gray-900 min-h-screen">
      ...
      {/* Back to dashboard */}
      <a
        href={`/dashboard/workspace/${params.id}/validation/landing`}
        className="fixed bottom-4 right-4 flex items-center gap-2 rounded-full bg-gray-900/80 backdrop-blur-sm px-4 py-2 text-xs text-white/90 shadow-lg hover:bg-gray-900 transition-colors"
      >
        <ArrowLeft className="h-3.5 w-3.5" /> Revenir a mon dashboard
      </a>
    </div>
  )
}
